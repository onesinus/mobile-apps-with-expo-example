...
import { screenHeight } from "../helpers/deviceInfo";
...
export default function MainScreen({
  ...
  const blankPicture = require('../assets/blank_avatar.png');
  ...
  const onClickList = (dataDetail) => {
    dataDetail.blankPicture = blankPicture;
  ...
  return (
    ...
    >
      ...
      {
        historyAttendance && historyAttendance.length < 1 && 
        <View 
          style={{ 
            flex: 1, 
            flexDirection: 'row', 
        ...
        >
            ...
            >
              Welcome to Online Attendance Apps
            </Text>
        </View>
      }
      {
              ...
              <View key={data["_id"]}>
                {
                  ...
                  <ListItem 
                    thumbnail
                    onPress={() => onClickList(data.AttendanceOut)}
                  >
                    <Left>
                      <Thumbnail 
                        source={data.AttendanceOut.imgUrl ? {uri: data.AttendanceOut.imgUrl}: blankPicture} 
                      />
                    </Left>
                ...
                <ListItem 
                  avatar
                  onPress={() => onClickList(data)}
                >
                  <Left>
                    <Thumbnail 
                      source={data.imgUrl ? {uri: data.imgUrl}: blankPicture} 
                    />
                  </Left>
                  <Body style={listBodyStyle}>
